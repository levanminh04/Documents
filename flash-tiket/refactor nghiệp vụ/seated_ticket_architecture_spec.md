...
*   Nếu Event **không dùng sơ đồ**: Form hiển thị các trường cơ bản (tên, giá, số lượng). Không hiển thị dropdown khán đài.
...
*   **Trường giới hạn mua (Max Tickets Per Order):** Trên giao diện form Tạo/Sửa Vé (cho cả hai trường hợp có dùng sơ đồ hay không), Frontend bắt buộc phải thiết kế ô nhập giới hạn số vé tối đa cho mỗi giao dịch (`max_per_order`, kiểu số nguyên, mặc định là 10) để map vào thuộc tính `max_per_order` trong database.
